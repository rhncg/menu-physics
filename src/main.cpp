...
class $modify(ModCreatorLayer, CreatorLayer)
{
    ...
    bool init()
    {
        ...
        float gravityY = static_cast<float>(Mod::get()->getSettingValue<double>("physics-gravity"));
        ...
        auto menu = typeinfo_cast<CCMenu *>(this->getChildByID("creator-buttons-menu"));
        if (menu)
        {
            menu->schedule(schedule_selector(ModCreatorLayer::customPhysicsUpdate));
            ...
            if (exitMenu)
            {
                auto toggler = CCMenuItemToggler::createWithStandardSprites(
                    this,
                    menu_selector(ModCreatorLayer::onTogglePhysics),
                    0.7f);
                ...
                if (auto layout = typeinfo_cast<AxisLayout *>(exitMenu->getLayout()))
                {
    ...
    void customPhysicsUpdate(float dt)
    {
        ...
        if (!creatorLayer->m_fields->m_buttonsCaptured)
        {
            ...
            CCPoint topRight = menu->convertToNodeSpace(CCPoint{winSize.width, winSize.height});

            b2BodyDef wallBodyDef;
            wallBodyDef.position.Set(0, 0);
            b2Body *staticWalls = creatorLayer->m_fields->m_world->CreateBody(&wallBodyDef);
            ...
            wallShape.SetTwoSided(b2Vec2(botLeft.x / PTM_RATIO, botLeft.y / PTM_RATIO), b2Vec2(topRight.x / PTM_RATIO, botLeft.y / PTM_RATIO));
            staticWalls->CreateFixture(&wallFixture);

            wallShape.SetTwoSided(b2Vec2(botLeft.x / PTM_RATIO, topRight.y / PTM_RATIO), b2Vec2(topRight.x / PTM_RATIO, topRight.y / PTM_RATIO));
            staticWalls->CreateFixture(&wallFixture);

            wallShape.SetTwoSided(b2Vec2(botLeft.x / PTM_RATIO, botLeft.y / PTM_RATIO), b2Vec2(botLeft.x / PTM_RATIO, topRight.y / PTM_RATIO));
            staticWalls->CreateFixture(&wallFixture);

            wallShape.SetTwoSided(b2Vec2(topRight.x / PTM_RATIO, botLeft.y / PTM_RATIO), b2Vec2(topRight.x / PTM_RATIO, topRight.y / PTM_RATIO));
            staticWalls->CreateFixture(&wallFixture);
            ...
            if (children && children->count() > 0)
            {
                ...
                float customFriction = static_cast<float>(Mod::get()->getSettingValue<double>("physics-friction"));
                float customBounciness = static_cast<float>(Mod::get()->getSettingValue<double>("physics-bounciness"));

                for (int i = 0; i < children->count(); ++i)
                {
                    auto btn = typeinfo_cast<CCNode *>(children->objectAtIndex(i));
                    if (btn)
                    {
                        ...
                        b2Body *body = creatorLayer->m_fields->m_world->CreateBody(&bodyDef);

                        b2PolygonShape boxShape;
                        boxShape.SetAsBox((w * 0.5f) / PTM_RATIO, (h * 0.5f) / PTM_RATIO);
                        ...
                        b2Vec2 randomImpulse(forceX(gen) * mass, forceY(gen) * mass);
...
{
    if (!m_creatorLayer || !m_creatorLayer->m_fields->m_world || !m_creatorLayer->m_fields->m_buttonsCaptured)
        return false;

    if (!m_creatorLayer->m_fields->m_physicsDraggingEnabled)
        return false;

    auto menu = typeinfo_cast<CCMenu *>(m_creatorLayer->getChildByID("creator-buttons-menu"));
    if (!menu)
        return false;

    CCPoint localClickPos = menu->convertToNodeSpace(touch->getLocation());
    b2Vec2 physicsTargetPos(localClickPos.x / PTM_RATIO, localClickPos.y / PTM_RATIO);
    ...
    for (auto &bp : m_creatorLayer->m_fields->m_physicsButtons)
    {
        ...
        for (b2Fixture *f = bp.body->GetFixtureList(); f; f = f->GetNext())
        {
            if (f->TestPoint(physicsTargetPos))
            {
                ...
                m_creatorLayer->m_fields->m_mouseJoint = static_cast<b2MouseJoint *>(m_creatorLayer->m_fields->m_world->CreateJoint(&md));
...
{
    bool isDragging = (m_creatorLayer && m_creatorLayer->m_fields->m_mouseJoint);
    if (!isDragging)
        return;
...
void TouchPhysicsHandler::ccTouchEnded(CCTouch *touch, CCEvent *event)
{
    ...
    {
        m_creatorLayer->m_fields->m_world->DestroyJoint(m_creatorLayer->m_fields->m_mouseJoint);
